...
def api_update_mobile_restriction(user_session, guild_id):
    ...
    try:
        app.logger.info(f"🔧 Mobile restriction API called for guild {guild_id}")
        
        # Verify user has access
        guild = verify_guild_access(user_session, guild_id)
        if not guild:
            app.logger.warning(f"❌ Access denied for guild {guild_id}")
            return jsonify({'success': False, 'error': 'Access denied'}), 403
        
        # Get mobile restriction setting from request
        data = request.get_json()
        if data is None:
            app.logger.error(f"❌ Missing data in request for guild {guild_id}")
            return jsonify({'success': False, 'error': 'Missing data'}), 400
        
        restrict_mobile = bool(data.get('restrict_mobile', False))
        app.logger.info(f"📱 Setting mobile restriction to {restrict_mobile} for guild {guild_id}")
        ...
        with get_db() as conn:
            ...
            if exists:
                app.logger.info(f"🔄 Updating existing record for guild {guild_id}")
                conn.execute(
                    "UPDATE server_subscriptions SET restrict_mobile_clockin = %s WHERE guild_id = %s",
                    (restrict_mobile, int(guild_id))
                )
            else:
                app.logger.info(f"➕ Inserting new record for guild {guild_id}")
            ...
            if verify_result:
                app.logger.info(f"✅ Verified database value: {verify_result['restrict_mobile_clockin']} for guild {guild_id}")
