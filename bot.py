...
async def setup_timeclock(interaction: discord.Interaction, channel: Optional[discord.TextChannel] = None, force_refresh: bool = False):
    ...
    if force_refresh:
        await interaction.edit_original_response(content=f"🔄 Refreshed timeclock in {channel_mention}.")
    else:
...
@tree.command(name="owner_refresh_all", description="[OWNER] Refresh timeclock messages across all servers")
async def owner_refresh_all(interaction: discord.Interaction):
    """Owner command to refresh timeclock messages across all servers (for deployments)"""
    if interaction.user.id != BOT_OWNER_ID:
        await send_reply(interaction, "❌ Access denied.", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.followup.send(f"🔄 Starting global refresh across {total_guilds} servers...", ephemeral=True)
        
        for guild in bot.guilds:
            try:
                # Get guild's current timeclock setup
                old_channel_id = get_guild_setting(guild.id, "button_channel_id")
                old_message_id = get_guild_setting(guild.id, "button_message_id")
                
                if not old_channel_id or not old_message_id:
                    results.append(f"⏭️ **{guild.name}**: No timeclock setup found")
                    skipped_count += 1
                    continue
                
                # Get the channel
                channel = bot.get_channel(old_channel_id)
                if not channel:
                    results.append(f"❌ **{guild.name}**: Channel not found (deleted?)")
                    failed_count += 1
                    continue
                
                # Check if bot has permissions
                if not channel.permissions_for(guild.me).send_messages:
                    results.append(f"❌ **{guild.name}**: Missing send permissions in #{channel.name}")
                    failed_count += 1
                    continue
                ...
                view = TimeClockView(guild_id=guild.id)
                new_message = await channel.send("**Time Clock** — Click a button to record your time.\n(Only you see confirmations.)", view=view)
                
                # Update settings
                set_guild_setting(guild.id, "button_channel_id", channel.id)
                set_guild_setting(guild.id, "button_message_id", new_message.id)
                
                results.append(f"✅ **{guild.name}**: Refreshed in #{channel.name}")
                refreshed_count += 1
                
            except Exception as e:
                results.append(f"❌ **{guild.name}**: Error - {str(e)[:50]}...")
                failed_count += 1
        
        # Send summary
        summary = f"🎯 **Global Refresh Complete**\n\n"
        summary += f"📊 **Summary:**\n"
        summary += f"• **Total Servers:** {total_guilds}\n"
        summary += f"• **✅ Refreshed:** {refreshed_count}\n"
        summary += f"• **⏭️ Skipped:** {skipped_count} (no setup)\n"
        summary += f"• **❌ Failed:** {failed_count}\n\n"
        
        # Include details (limit to prevent message too long)
        if results:
            summary += f"📋 **Details:**\n"
            # Show first 10 results to avoid message limits
        ...
        await interaction.followup.send(f"❌ Global refresh failed: {str(e)}", ephemeral=True)
