...
- **Server Visibility Filtering**: Dashboard only shows servers where (bot_access_paid = 1) OR (bot is currently in the server). Servers that are unpaid AND where the bot has left/was never invited are completely hidden from all lists, stats, and webhook events. This ensures owner only manages active/paid servers.
...
- **Manual Subscription Management**: Owner can manually grant AND revoke bot access ($5 one-time) and retention tiers (7-day/$5, 30-day/$10) to any server via dashboard buttons. Grant buttons disable after granting; revoke buttons (red/danger style) disable when nothing to revoke. Revoke actions require confirmation dialog. All manual grants tracked in database with granted_by user ID and granted_at timestamp. API endpoints `/api/owner/grant-access` and `/api/owner/revoke-access` enforce owner-only access. Revoking bot access also clears retention tier and sets status to 'cancelled'.
- **Guild Name Resolution**: When server names are missing from `bot_guilds` table, owner dashboard automatically fetches guild names from Discord API and caches them in database. Shows "Unknown Server (ID: xxx)" with guild ID for servers the bot cannot access (403) or that no longer exist (404).
- **Server Management**: View all visible servers (paid or bot-present) with subscription details, active session counts, Stripe customer/subscription IDs. Search by name or ID. Filter by: All, Paid, Unpaid, Free Tier, 7-Day, 30-Day, Past Due.
